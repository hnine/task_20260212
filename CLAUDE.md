...
- API Endpoints
  ...
  check duplicated employee data (same name, email, tel number) if same employee data is exist, return error message
- Create test code for api tests
  More details on TDD Workflow
- Create log system
  Using Serilog
  Using Singletone pattern for logging
  Log format configuration on appsettings.json
  Default log format ([{Timestamp:yyyy-MM-dd HH:mm:ss.fff}] - {Message:lj} [{CallerMethod}]{NewLine}{Exception})
  Log schema
  API call histories
  System initialize and fail point
  Performance monitoring
- Using CORS
  Allow frontend (default http://localhost:5173, http://localhost:3000)
  Can configuration on appsettings.json
  Change open port setting on appsettings.json
- API Spec (using OpenAPI)
  Can configuration on appsettings.json
  Change open port setting on appsettings.json (default 5086)
